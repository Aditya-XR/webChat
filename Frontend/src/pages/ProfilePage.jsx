...
const ProfilePage = () => {
  ...
  const {authUser, updateProfile, requestEmailChange} = useContext(AuthContext);
  ...
  const handleEmailChangeRequest = async (e) => {
    ...
    const didRequestEmailChange = await requestEmailChange(newEmail);
  ...
  return (
    <div className='min-h-screen bg-cover bg-no-repeat flex items-center justify-center'>
        <div className='w-5/6 max-w-4xl backdrop-blur-2xl text-gray-300 border-2
        border-gray-600 flex items-center justify-between gap-8 max-lg:flex-col-reverse
        rounded-lg p-6'>
          <div className='flex-1 w-full flex flex-col gap-6'>
            ...
            <form onSubmit={handleEmailChangeRequest} className='flex flex-col gap-4 border border-gray-700 rounded-lg p-4'>
              <h3 className='text-lg'>Email Address</h3>
              <input
                type="email"
                value={authUser?.email || ""}
                readOnly
                className='p-2 border border-gray-600 rounded-md bg-black/20 text-gray-300 cursor-not-allowed'
              />
              {authUser?.pendingEmail && (
                <p className='text-sm text-amber-300'>
                  Pending verification: {authUser.pendingEmail}
                </p>
              )}
              <input
              ...
              />
              <button type='submit' className='bg-emerald-600 hover:bg-emerald-700 transition-colors text-white py-2 rounded-md'>
            ...
            hover:text-white transition-colors py-2 rounded-md'>Go Back</button>
          </div>
          <img src={selectedImage ? URL.createObjectURL(selectedImage) : authUser?.profilePic || assets.logo_icon} alt="" className='max-w-[180px] my-5 rounded-full mx-auto' />
        </div>
    </div>
  )
}
